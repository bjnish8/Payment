import "./styles.scss";
import React, { useState, useEffect} from "react";
import { loadStripe } from "@stripe/stripe-js";
import {
  CardNumberElement,
  CardExpiryElement,
  CardCvcElement,
  Elements,
  useStripe,
  useElements,
} from "@stripe/react-stripe-js";

const stripePromise = loadStripe(process.env.REACT_APP_STRIPE_PUBLISHABLE_SECRET);
const email_regex = /[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?/;

const CARD_OPTIONS = {
  style: {
    base: {
      color: "#32325D",
      fontWeight: 500,
      fontSize: "16px",
      fontSmoothing: "antialiased",

      "::placeholder": {
        color: "#CFD7DF",
      },
      ":-webkit-autofill": {
        color: "#e39f48",
      },
    },
    invalid: {
      iconColor: "#ed4337",
      color: "#ed4337",
    },
  },
};
const SubmitButton = ({ processing, error, children, disabled }) => (
  <button
    className={`SubmitButton ${error ? "SubmitButton--error" : ""}`}
    type="submit"
    disabled={processing || disabled}
  >
    {processing ? "Processing..." : children}
  </button>
);

const ResetButton = ({ onClick }) => (
  <button type="button" className="ResetButton" onClick={onClick}>
    <svg width="32px" height="32px" viewBox="0 0 32 32">
      <path
        fill="#FFF"
        d="M15,7.05492878 C10.5000495,7.55237307 7,11.3674463 7,16 C7,20.9705627 11.0294373,25 16,25 C20.9705627,25 25,20.9705627 25,16 C25,15.3627484 24.4834055,14.8461538 23.8461538,14.8461538 C23.2089022,14.8461538 22.6923077,15.3627484 22.6923077,16 C22.6923077,19.6960595 19.6960595,22.6923077 16,22.6923077 C12.3039405,22.6923077 9.30769231,19.6960595 9.30769231,16 C9.30769231,12.3039405 12.3039405,9.30769231 16,9.30769231 L16,12.0841673 C16,12.1800431 16.0275652,12.2738974 16.0794108,12.354546 C16.2287368,12.5868311 16.5380938,12.6540826 16.7703788,12.5047565 L22.3457501,8.92058924 L22.3457501,8.92058924 C22.4060014,8.88185624 22.4572275,8.83063012 22.4959605,8.7703788 C22.6452866,8.53809377 22.5780351,8.22873685 22.3457501,8.07941076 L22.3457501,8.07941076 L16.7703788,4.49524351 C16.6897301,4.44339794 16.5958758,4.41583275 16.5,4.41583275 C16.2238576,4.41583275 16,4.63969037 16,4.91583275 L16,7 L15,7 L15,7.05492878 Z M16,32 C7.163444,32 0,24.836556 0,16 C0,7.163444 7.163444,0 16,0 C24.836556,0 32,7.163444 32,16 C32,24.836556 24.836556,32 16,32 Z"
      />
    </svg>
  </button>
);

const Loader = () => <div className="Loader">
<div className="Spinner"/>
  <span> Processing Payment... </span>
</div>

const ErrorMessage = ({ children }) => (
  <div className="ErrorMessage" role="alert">
    <svg width="21" height="21" viewBox="0 0 17 17">
      <path
        fill="#FFF"
        d="M8.5,17 C3.80557963,17 0,13.1944204 0,8.5 C0,3.80557963 3.80557963,0 8.5,0 C13.1944204,0 17,3.80557963 17,8.5 C17,13.1944204 13.1944204,17 8.5,17 Z"
      />
      <path
        fill="#ff0000"
        d="M8.5,7.29791847 L6.12604076,4.92395924 C5.79409512,4.59201359 5.25590488,4.59201359 4.92395924,4.92395924 C4.59201359,5.25590488 4.59201359,5.79409512 4.92395924,6.12604076 L7.29791847,8.5 L4.92395924,10.8739592 C4.59201359,11.2059049 4.59201359,11.7440951 4.92395924,12.0760408 C5.25590488,12.4079864 5.79409512,12.4079864 6.12604076,12.0760408 L8.5,9.70208153 L10.8739592,12.0760408 C11.2059049,12.4079864 11.7440951,12.4079864 12.0760408,12.0760408 C12.4079864,11.7440951 12.4079864,11.2059049 12.0760408,10.8739592 L9.70208153,8.5 L12.0760408,6.12604076 C12.4079864,5.79409512 12.4079864,5.25590488 12.0760408,4.92395924 C11.7440951,4.59201359 11.2059049,4.59201359 10.8739592,4.92395924 L8.5,7.29791847 L8.5,7.29791847 Z"
      />
    </svg>
    {children}
  </div>
);

const SplitFieldsForm = ({ total }) => {
  const stripe = useStripe();
  const elements = useElements();
  const [formError, setFormError] = useState(null);
  const [cardError, setCardError] = useState(null);
  const [cardComplete, setCardComplete] = useState(false);
  const [processing, setProcessing] = useState(false);
  const [paymentMethod, setPaymentMethod] = useState(null);
  const [billingDetails, setBillingDetails] = useState({
    email: "",
    name: "",
  });
  const [inProgress, setInProgress] = useState(false)

  useEffect(() => {
    setFormError(null)
  }, [billingDetails])

  const userInfoValidate = () => {
      if (billingDetails.name.length === 0){
          setFormError({id:'name', message:'Please provide your name'})
        } else if(billingDetails.email.length === 0){
            setFormError({id:'email', message:'Please provide an email'})
        } 
        else if(!email_regex.test(billingDetails.email.toLowerCase())){
            setFormError({id:'email',message:'Please provide a valid email'})
        }   
        else {
            setFormError(null)
            return true
        }
    }
  const handleChange = (e) => {
    setCardError(e.error);
    setCardComplete(e.complete);
  };

  const handleBillingChange = (obj) => {
      setBillingDetails({...billingDetails, ...obj})
      userInfoValidate()
  }

  const handleSubmit = async (event) => {
    event.preventDefault();

    if (!stripe || !elements) {
      // Stripe.js has not loaded yet. Make sure to disable
      // form submission until Stripe.js has loaded.
      return;
    }

    if (userInfoValidate()){
        return
    }

    const cardElement = elements.getElement(CardNumberElement);

    if (cardError) {
      cardElement.focus();
      return;
    }

    if (cardComplete) {
      setProcessing(true);
    }

    const payload = await stripe.createPaymentMethod({
      type: "card",
      card: cardElement,
      billing_details: billingDetails,
    });

    setProcessing(false);

    if (payload.error) {
      setCardError(payload.error);
    } else {
      setPaymentMethod(payload.paymentMethod);
    }
  };
  const reset = () => {
    setCardError(null);
    setProcessing(false);
    setPaymentMethod(null);
    setBillingDetails({
      email: "",
      name: "",
    });
  };
  return paymentMethod ? (
    <div className="Result">
      <div className="ResultTitle" role="alert">
        Payment successful
      </div>
      <div className="ResultMessage">
        Thanks for trying Stripe Elements. No money was charged, but we
        generated a PaymentMethod: {paymentMethod.id}
      </div>
      <ResetButton onClick={reset} />
    </div>
  ) : (
    <div className="CheckoutDiv">
      <form className={inProgress?"invisible":""} onSubmit={handleSubmit}>
        <h4> Personal Details </h4>
        <label>
          <span>Full name</span>
          <input name="name" id="name" type="text" placeholder="John Doe" onChange={(e) => {
              handleBillingChange({name:e.target.value})
          }}/>
        </label>
        <label>
          <span>Email address</span>
          <input name="email" id="email" type="text" placeholder="johndoe@email.com" onChange={(e) => {
              handleBillingChange({email:e.target.value})
          }}/>
        </label>
        <h4> Payment Details </h4>
        <label>
          <span>Card number</span>
          <CardNumberElement options={CARD_OPTIONS} onChange={handleChange} />
        </label>
        <div className="FormRow">
        <label className="row-small">
          <span>Expiration</span>
          <CardExpiryElement options={CARD_OPTIONS} onChange={handleChange} />
        </label>
          <label className="row-small">
            <span>CVC</span>
            <CardCvcElement
              options={CARD_OPTIONS}
              onChange={handleChange}
              className="WidthSmall"
            />
          </label>
        </div>
        {formError && <ErrorMessage>{formError.message}</ErrorMessage>}
        {cardError && <ErrorMessage>{cardError.message}</ErrorMessage>}
        <SubmitButton processing={processing} error={cardError} disabled={!stripe}>
          Pay ${total}
        </SubmitButton>
      </form>
      {inProgress && <Loader />}
      <button onClick={() => {
          setInProgress(true)
          setTimeout(() => {
          setInProgress(false)
      }, 2000);}}>Show Spinner </button>
    </div>
  );
};

const App = ({ total }) => {
  return (
    <Elements stripe={stripePromise}>
      <SplitFieldsForm total={total} handleResult={() => {}} />
    </Elements>
  );
};

export default App;
